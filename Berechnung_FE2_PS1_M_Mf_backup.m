...
M_take_off_initial.M_fuel = Ergebnis_basis_m.m_To * FF.Kappa_ges;
...
% Formel (8-18) S.283
NR_W_sc.W_sc_initial = (NR_W_sc.k_sc(3,1) .* Ergebnis_basis_m.m_To.^(2/3)) .* 0.768; % 0.768 sind fuer umrechnungsfaktor zu kg
% +20% masse fuer leading edge flap +15% masse lift tumper controls
NR_W_sc.m_cockpitcontrols = 50; % [kg] Formel (8-19)
NR_W_sc.m_autopilot = 9 * Ergebnis_basis_m.m_To.^(1/5); % [kg] Formel (8-20)

% tabelle 8-7 S 284
NR_W_sc.m_maneuver_control = 0.773 * Ergebnis_basis_m.m_To^(0.60);
...
% Instrumente S.289
NR_Airframe_service.W_ieg_alt = 0.347 * W_DeliveryEmpty^(5/9) * specs.max_range_basis_km^(1/4);
% ????????????????????????????????????????????????????????????????????
...
NR_Airframe_service.W_flight_deck_acc = 16.5 * W_DeliveryEmpty^(0.285);

% PAX Cabin Accomodities S.291

% Pax Sitze gemäß Torenbeek Table 3-2 S.76
NR_Airframe_service.seats = [[10.9 21.3 29.9]; [13.6 25.4 35.4]; [21.3 31.8 0]];
...
M_Airframe_service_and_equipment.M_Miscellaneous = 0.005 * Ergebnis_basis_m.m_To;
