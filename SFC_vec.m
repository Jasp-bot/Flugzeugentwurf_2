function [sfc_daNh, sfc_1PERh, sfc_1PERs] = SFC_vec(Height,MachNumber,BPR)
% clc
% clear all
% close all

% Height = round(linspace(1, 13000, 10)).';
% MachNumber = linspace(0,1,100);
% BPR = 10;
...
%% Atmospheric Parameters
   ...
   % [a, p, rho, T, relativeRho, relativePressure] = f_atmosphere(Height, ISAPlus);    
    ...
    p(x,1) = ISA.p(Height(x,1));
    a(x,1) = ISA.a(Height(x,1));
    rho(x,1)= ISA.rho(Height(x,1));
    T(x,1) = ISA.T(Height(x,1));
    relativeRho(x,1) = ISA.rel_rho(Height(x,1));
    relativePressure(x,1) = p(x,1)/ISA.p0;
