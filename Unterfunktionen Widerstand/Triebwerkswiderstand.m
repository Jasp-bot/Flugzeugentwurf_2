...
function [c_w_TW] = Triebwerkswiderstand(v_air, alpha_Rumpf_grad, hoehe)
...
k_TW = 0.2;
Re_TW = FUN.Re_H_fun(Annahmen.l_TW, v_air, hoehe); %(Annahmen.l_TW * (specs.Ma_CR * ISA.a(Annahmen.hoehe_CR))) / (ISA.kin_visk(Annahmen.hoehe_CR));
...
Re_u_Py = FUN.Re_H_fun(Annahmen.x_u_Py, v_air, hoehe);  %Annahmen.x_u_HLW * v_air / ISA.kin_visk(Annahmen.hoehe_CR);

Re_Py = FUN.Re_H_fun(specs.lh_TW, v_air, hoehe);    % HLW.Fluegeltiefen_eta_oR .* v_air ./ ISA.kin_visk(Annahmen.hoehe_CR);
